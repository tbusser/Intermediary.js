	...
	*/
	...
	Channel.prototype = {
		...
		addChannel: function(channelName) {
			// 1: If the current channel has a namespace we need to add a ':' to it and
			...
			// 2: Add the new channel to the list of child channels. The parent
			//   of the new channel will be the current channel
			var namespace = ((this.namespace === '') ? '' : this.namespace + ':') + channelName,  /* [1] */
			    newChannel = new Channel(namespace, this, channelName);                           /* [2] */
	...
	return {
		...
		publish: function(namespace) {
			...
			// Check if there was a message to pass along, if not push null into
			// args so the namespace will still be the second argument
			if (args.length === 0) {
				args.push(null);
			}
